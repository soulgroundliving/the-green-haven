...
exports.cleanupResolvedComplaints = functions.region('asia-southeast1').pubsub
  ...
  .onRun(async (context) => {
    try {
      console.info('🧹 Cleaning up resolved complaints...');
      ...
      for (const buildingDoc of buildingsSnap.docs) {
        ...
        for (const roomDoc of roomsSnap.docs) {
          ...
          if (archivedCount > 0) {
            console.info(`✅ Archived ${archivedCount} complaints from ${buildingId}/${roomId}`);
          }
        }
      }
...
exports.awardComplaintFreeMonth = functions.region('asia-southeast1').pubsub
  ...
  .onRun(async () => {
    ...
    console.info('🎯 awardComplaintFreeMonth:', JSON.stringify(result));
    return result;
  });
...
async function _runCheckAndAwardBadgesPlayer(tenantId, authToken) {
  ...
  if (toAward.length > 0) {
    ...
    toAward.forEach(b => console.info(`🎖️ Awarded "${b.label}" to player ${tenantId}`));
  }
...
exports.checkAndAwardBadges = functions.region('asia-southeast1').https.onCall(async (data, context) => {
  try {
    ...
    if (toAward.length > 0) {
      ...
      toAward.forEach(b => console.info(`🎖️ Awarded "${b.label}" to ${building}/${roomId}`));
    }
