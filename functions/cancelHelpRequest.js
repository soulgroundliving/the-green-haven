/**
 * cancelHelpRequest — the requester cancels their own help request, or an admin
 * cancels an abusive one (Meaning Layer #2 · the admin-monitor moderation
 * action). Allowed only from a non-terminal state (open | accepted); done /
 * already-cancelled requests are immutable. No points move on cancel.
 *
 * §7-NN callable. Region asia-southeast1.
 */
const functions = require('firebase-functions/v1');
const admin = require('firebase-admin');
const { canCancel } = require('./_helpRequestEngine');

if (!admin.apps.length) admin.initializeApp();
const firestore = admin.firestore();

exports.cancelHelpRequest = functions.region('asia-southeast1').https.onCall(async (data, context) => {
