/**
 * Unit tests for cleanupPetPlaydatesScheduled._run — the Pet Playdate (#11)
 * housekeeping sweep. Deletes playdates whose expiresAt passed more than
 * GRACE_MS ago (one single-field query, no composite index), keeps within-grace
 * + fresh.
 */
'use strict';
...
const cf = require('../cleanupPetPlaydatesScheduled');

after(() => { Module._load = _origLoad; });

describe('cleanupPetPlaydatesScheduled._run', () => {
  beforeEach(reset);

  it('deletes playdates expired > GRACE_MS ago; keeps within-grace + fresh', async () => {
    const now = Date.now();
    ...
    docs.fresh = { expiresAt: now + 3600 * 1000 };           // open → keep

    const res = await cf._run();
    assert.equal(res.deleted, 2);
    assert.ok(!('old1' in docs) && !('old2' in docs), 'stale playdates deleted');
