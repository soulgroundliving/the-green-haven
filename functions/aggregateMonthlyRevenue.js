 ...
 *   updatedAt: ...,
 *   months: {
 *     1: { rentIncome, electricIncome, waterIncome, trashIncome, otherIncome, totalRevenue,
 *          paidCount, paidRevenue, pendingCount, pendingRevenue,
 *          byBuilding: { rooms: {...}, nest: {...} } },
 *     2: {...}, ..., 12: {...}
 *   },
 *   annual: { rentIncome, electricIncome, waterIncome, trashIncome, otherIncome, totalRevenue,
...
function _emptyByBuilding(buildings) {
  ...
  for (const b of buildings) {
    out[b] = { rent: 0, electric: 0, water: 0, trash: 0, other: 0, total: 0, paid: 0, pending: 0 };
  }
  return out;
}
function _emptyMonth(buildings) {
  return {
    rentIncome: 0, electricIncome: 0, waterIncome: 0, trashIncome: 0, otherIncome: 0,
    totalRevenue: 0,
...
async function aggregateYear(yearBE) {
  ...
  for (const building of buildings) {
    ...
    for (const roomId of Object.keys(rooms)) {
      ...
      for (const billId of Object.keys(billsObj)) {
        ...
        // (bill total minus the 4 named categories) so categories always sum to total.
        const other = Math.max(0, total - rent - elec - water - trash);
        ...
        if (bb) {
          bb.rent += rent; bb.electric += elec; bb.water += water; bb.trash += trash; bb.other += other;
          bb.total += total;
  ...
  for (let m = 1; m <= 12; m++) {
    ...
    buildings.forEach(b => {
      ...
      if (!a || !s) return;
      ['rent','electric','water','trash','other','total','paid','pending'].forEach(k => { a[k] += s[k]; });
    });
  }
