...
exports.claimWellnessQuizPoints = functions.region('asia-southeast1').https.onCall(async (data, context) => {
  ...
  if (reqTenantId && !building && !roomId) {
    ...
    try {
      ...
      console.log(`🎯 Player wellness quiz: ${tenantId}/${aid} passed=${grade.passed} +${result.reward}`);
      return {
        success: true,
        passed: grade.passed,
        score: grade.correct,
  ...
  try {
    ...
    console.log(`🎯 Wellness quiz: ${canonicalBuilding}/${roomId}/${aid} passed=${grade.passed} +${result.reward}`);
    return {
      success: true,
      passed: grade.passed,
      score: grade.correct,
