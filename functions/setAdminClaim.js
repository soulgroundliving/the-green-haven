...
exports.setAdminClaim = functions.region('asia-southeast1').https.onRequest(async (req, res) => {
  ...
  if (initToken && envToken && initToken === envToken) {
    ...
    authed = true;
    console.info('🔑 setAdminClaim: bootstrap via INIT_TOKEN (no admin yet)');
  }
  ...
  if (!authed) {
    ...
    try {
      ...
      authed = true;
      console.info(`🔑 setAdminClaim: called by existing admin ${decoded.email}`);
    } catch (e) {
  ...
  try {
    ...
    console.info(`✅ setAdminClaim: ${JSON.stringify(claims)} set on uid=${user.uid} email=${email}`);
