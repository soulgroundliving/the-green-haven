...
exports.liffSignIn = functions
  ...
  if (liffData.role === 'player') {
    ...
    try {
      ...
      console.info(`✅ liffSignIn (player): uid=${uid} tenantId=${tenantId} LINE ${lineUserId}`);
    } catch (e) {
  ...
  try {
    ...
    console.info(`✅ liffSignIn: uid=${uid} → ${building}/${room}${tenantId ? ' tenantId=' + tenantId : ''} (LINE ${lineUserId})`);
  } catch (e) {
