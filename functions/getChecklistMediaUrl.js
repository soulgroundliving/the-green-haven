 ...
 *
 *   After path-auth passes, the instance doc at the requested instanceId
 *   MUST still live at the same building+roomId — protects against
 *   directory-traversal-like reads of another room's checklist via the
 *   same auth token. Admin + managedBuildings paths skip the instance
 *   check (they can read any path within their scope).
 *
 ...
 * @see getLeaseDocUrl.js — same template (no instance check)
 */
...
exports.getChecklistMediaUrl = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    // 6-path auth gate via _authSoT helper. Returns the viaPath identifier
    // so we know whether to still require the instance-doc check below
    // (admin + manager bypass it; tenant paths require it).
    const { viaPath } = await assertTenantAccess({
      building, roomId,
      context, firestore,
      HttpsError: functions.https.HttpsError,
    });

    // Instance check — required for non-admin paths. Defends against a
    // tenant of room 15 reading /checklists/rooms/15/<instance-from-room-16>/...
    // via a forged path. Admin + managedBuildings skip (they can read any
    // valid path within their scope).
    if (viaPath !== 'admin' && viaPath !== 'manager') {
