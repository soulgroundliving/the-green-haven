...
| `npm run test:rules` | Firestore security rules (261 cases) |
| `npm run test:storage` | Storage security rules (42 cases) |
...
│   └── __tests__/            # 95 CF unit-test files
├── api/                      # Vercel serverless functions
├── firestore.rules + firestore.rules.test.js  # 261 rule tests
├── storage.rules + storage.rules.test.js      # 42 rule tests
...
- Recurring anti-patterns → logged in CLAUDE.md §7 (currently A–YY, 51 patterns)
...
- [CLAUDE.md](CLAUDE.md) — workflow protocol, tech stack details, anti-pattern catalog (§7 A–YY)
