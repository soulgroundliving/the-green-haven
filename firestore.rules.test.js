...
describe('tenants — sensitive field protection', () => {
  ...
  it('LIFF-linked tenant CANNOT fake their own kindnessTier (Meaning Layer #6 §6 tamper-proof)', async () => {
    // kindnessTier is server-mirrored from trustScores by the trust sweep CF (the
    // SAME combined mirror write as reputationTier). It's in the protected
    // affectedKeys block so a self-owned tenant can't fake their own kindness tier
    // via devtools — even though they own the doc.
    await seedTenant({ linkedAuthUid: 'linked-uid-1' });
    const db = ANON('linked-uid-1').firestore();
    await assertFails(updateDoc(doc(db, 'tenants/rooms/list/101'), {
      kindnessTier: 'radiant'
    }));
  });
  ...
  it('LIFF-linked tenant CANNOT fake their own residentRankTier (Meaning Layer #8 §6 tamper-proof)', async () => {
    // residentRankTier is server-mirrored from trustScores by the trust sweep CF
    // (the SAME combined mirror write as the other three tiers). It's in the
    // protected affectedKeys block so a self-owned tenant can't promote themselves
    // to a top community rung via devtools — even though they own the doc.
    await seedTenant({ linkedAuthUid: 'linked-uid-1' });
    const db = ANON('linked-uid-1').firestore();
    await assertFails(updateDoc(doc(db, 'tenants/rooms/list/101'), {
      residentRankTier: 'taproot'
    }));
  });

  it('email admin CAN modify any tenant field including gamification + reputationTier + kindnessTier + verifiedHelperTier + residentRankTier', async () => {
    const db = EMAIL_ADMIN().firestore();
    await assertSucceeds(updateDoc(doc(db, 'tenants/rooms/list/101'), {
...
describe('communityRequests — building-scoped read, CF-only write (Meaning Layer #3)', () => {
  it('admin can read any building\'s community request', async () => {
    ...
    await assertSucceeds(getDoc(doc(EMAIL_ADMIN().firestore(), 'communityRequests/c1')));
  });

  it('LIFF tenant can read a request in their OWN building', async () => {
    await seedDoc('communityRequests/c1', { building: 'rooms', room: '101', status: 'open' });
    await assertSucceeds(getDoc(doc(LIFF_TENANT().firestore(), 'communityRequests/c1')));
  });

  it('LIFF tenant CANNOT read a request in a DIFFERENT building (PDPA scope)', async () => {
    await seedDoc('communityRequests/c1', { building: 'nest', room: 'N1', status: 'open' });
    await assertFails(getDoc(doc(LIFF_TENANT().firestore(), 'communityRequests/c1')));
  });

  it('anonymous user (no building claim) CANNOT read', async () => {
    await seedDoc('communityRequests/c1', { building: 'rooms', room: '101', status: 'open' });
    await assertFails(getDoc(doc(ANON().firestore(), 'communityRequests/c1')));
  });

  it('LIFF tenant CANNOT client-write — create or update (CF-only)', async () => {
    await assertFails(addDoc(collection(LIFF_TENANT().firestore(), 'communityRequests'),
      { building: 'rooms', room: '101', title: 'x', status: 'open' }));
    await seedDoc('communityRequests/c1', { building: 'rooms', status: 'open' });
    await assertFails(updateDoc(doc(LIFF_TENANT().firestore(), 'communityRequests/c1'), { status: 'offered' }));
  });

  it('admin CANNOT client-write either (every transition is a callable)', async () => {
    await assertFails(addDoc(collection(EMAIL_ADMIN().firestore(), 'communityRequests'),
      { building: 'rooms', title: 'x', status: 'open' }));
  });
});

describe('foodShares — building-scoped read, CF-only write (Meaning Layer #4)', () => {
  it('admin can read any building\'s food share', async () => {
    await seedDoc('foodShares/f1', { building: 'nest', room: 'N1', status: 'available' });
    await assertSucceeds(getDoc(doc(EMAIL_ADMIN().firestore(), 'foodShares/f1')));
  });

  it('LIFF tenant can read a share in their OWN building', async () => {
    await seedDoc('foodShares/f1', { building: 'rooms', room: '101', status: 'available' });
    await assertSucceeds(getDoc(doc(LIFF_TENANT().firestore(), 'foodShares/f1')));
  });

  it('LIFF tenant CANNOT read a share in a DIFFERENT building (PDPA scope)', async () => {
    await seedDoc('foodShares/f1', { building: 'nest', room: 'N1', status: 'available' });
    await assertFails(getDoc(doc(LIFF_TENANT().firestore(), 'foodShares/f1')));
  });

  it('anonymous user (no building claim) CANNOT read', async () => {
    await seedDoc('foodShares/f1', { building: 'rooms', room: '101', status: 'available' });
    await assertFails(getDoc(doc(ANON().firestore(), 'foodShares/f1')));
  });

  it('LIFF tenant CANNOT client-write — create or update (CF-only)', async () => {
    await assertFails(addDoc(collection(LIFF_TENANT().firestore(), 'foodShares'),
      { building: 'rooms', room: '101', title: 'x', status: 'available' }));
    await seedDoc('foodShares/f1', { building: 'rooms', status: 'available' });
    await assertFails(updateDoc(doc(LIFF_TENANT().firestore(), 'foodShares/f1'), { status: 'claimed' }));
  });

  it('admin CANNOT client-write either (every transition is a callable)', async () => {
    await assertFails(addDoc(collection(EMAIL_ADMIN().firestore(), 'foodShares'),
      { building: 'rooms', title: 'x', status: 'available' }));
  });
});
