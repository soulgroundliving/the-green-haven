...
- **Slice C core** — NEXT (rooms-building, human-damage only): itemized deductions `{desc, amount, photo}` + `DEPOSIT_RETURNED` audit (via `recordAdminAction` or a `settleDeposit` CF) + damage-photo & refund-slip Storage. Pet-cause routing deferred to Nest ~Aug. Plan = ACTIVE PLAN §SLICE C above. Verified current state: deduction shape today = `{reason, amount}` (no cause/photo); `_actionAudit.js` VALID_ACTIONS has **no** `DEPOSIT_RETURNED`; return refund is installment-aware (held−deductions); no deposit-slip Storage path exists.
