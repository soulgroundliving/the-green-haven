...
- [ ] **UX — live a11y verify on Vercel** (the §7-J close for items above): Chrome MCP on deployed dashboard (admin) + tenant_app (LIFF) — Tab+Enter through tiles, focus rings visible, tab `aria-selected` flips, contrast in light+dark. Unit gates green (248) + E2E live smoke green per merge; this is the visual confirmation.
