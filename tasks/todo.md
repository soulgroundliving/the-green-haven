...
- **Slice C core** ✅ SHIPPED to branch · **PR [#250](https://github.com/soulgroundliving/the-green-haven/pull/250)** (`bd8f67e`, +242/−25) · ⏳ awaiting merge+deploy. Rooms-building, human-damage only (pet-cause routing deferred to Nest ~Aug).
  ...
  - **Graceful degradation:** client ships on Vercel merge (settlement always saves); functions auto-deploy via `deploy-functions.yml` on merge (recordAdminAction audit); **photos need `firebase deploy --only storage`** (owner-gated). All upload/audit paths best-effort → no broken window pre-deploy.
  - **⏳ Open (owner):** merge PR #250 (= CF auto-deploy, money-flow §7-I) → `firebase deploy --only storage` (branch-check) → live-verify: admin settles test room w/ photo → `returned` + `refundSlip` stored + `DEPOSIT_RETURNED` audit row + tenant badge flips.
