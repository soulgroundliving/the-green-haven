...
- [~] **Security — move WAQI/IQAir tokens → Secret Manager** — ATTEMPTED + REVERTED 2026-06-01 (PR #216 merged → broke prod CF deploy: deploy SA can't access `IQAIR_API_KEY` value in Secret Manager → blocked all CF deploys → reverted `adae1cc`). Code was correct; **blocked on IAM** — grant the deploy SA + runtime SA `roles/secretmanager.secretAccessor` on both secrets, then test-deploy ONE CF before re-merging. See §7-WW + air_quality_waqi.md. NOTE: `functions/.env` is gitignored — hardening, not a leak.
