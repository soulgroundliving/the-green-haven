# §7 Anti-Patterns — Full Detail (incident history, code, debugging signatures)

On-demand companion to `CLAUDE.md` §7. Each `### X.` below is the full version of the
same-lettered **stub** in CLAUDE.md §7. The stub (always auto-loaded) carries: recognise +
rule + detection grep. THIS doc (pull when working in the area) carries: the incident, code
examples, debugging signatures, and family cross-refs.

Grep one: `grep -n "^### HH\." tasks/lessons_antipatterns.md` then read its block, or
`awk '/^### HH\./{p=1} p&&/^### [A-Z]+\. /&&!/^### HH\./&&NR>1{exit} p' tasks/lessons_antipatterns.md`.
...
> **⚠️ 2026-06-13 re-audit — the "live omits `blob:`" fact below is no longer current.** A header-drift audit (curl on login/dashboard/tenant_app) found the live CSP now MATCHES `vercel.json` **byte-for-byte**, including `img-src 'self' data: blob: https:` (blob: present) and all 24 `script-src` sha256 hashes. So the Vercel-UI header override is currently **DORMANT** and the CSP hash pipeline IS effective live. What stays true: (1) preview a File via `data:`/`FileReader` — narrower + works regardless of `img-src`; (2) **verify the LIVE header (curl), not `vercel.json`** — a UI *header* override CAN re-diverge, and it did on 2026-06-04. The 2026-06-04 incident below is kept as the dated record.

`vercel.json` lists `img-src 'self' data: blob: https:`, but on **2026-06-04** the **deployed** dashboard header was `img-src 'self' data: https:` — **no `blob:`** (a Vercel project Settings → Headers UI override, [[feedback_vercel_ui_overrides_json]]; dormant as of 2026-06-13 per the banner). So a grep of `vercel.json` is NOT proof of what the browser enforces — only the deployed header (or the runtime violation) is.
