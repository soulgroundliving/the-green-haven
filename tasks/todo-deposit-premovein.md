...
- [ ] **`functions/confirmMoveIn.js`** (NEW callable SE1) *(or fold into `convertBookingToTenant`)* — `reserved → holding`: set `lease.moveInDate` (§7-BBB occupancy, day-accurate, NOT contractStart) + activate tenant + `lease.status` + deposit status in **ONE batch** (§7-DD sibling writes). Admin preview → explicit click (§7-I), never auto. *Why: the atomic "actually moved in" gate.*
- [ ] **`functions/forfeitReservedDeposit.js`** (NEW callable SE1) *(or admin client write + audit)* — `reserved → forfeited`: `forfeitedAmount` = all paid, `DEPOSIT_FORFEITED` audit, NO refund (Q2). *Why: the not-guaranteed branch.*
...
- [ ] **`functions/index.js`** — register `confirmMoveIn` · `forfeitReservedDeposit` · `verifyDepositSlip` column-0 (§7-CCC/LLL). Tests (`Module._load` mock). *Why: CI deploy-list + global scope.*
