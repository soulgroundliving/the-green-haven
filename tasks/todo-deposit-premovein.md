...
- [x] ✅ **`functions/_billWrite.js`** (`00d5077`) — added `daysInMonth` + `moveInDay(tenantData)` + `isMoveInMonth(tenantData, billCeYM)` + pure `proratedMoveInRent(monthlyRent, day, billCeYM)` = `min(rent, round((rent/30) × daysOccupied))`, `daysOccupied = daysInMonth − moveInDay + 1`, **`daysOccupied ≤ 5 → 0`**, **capped at one full month** (day-1-of-31-day edge). `buildCanonicalBill({tenantData})` applies to the move-in month only; utilities/trash = actual; flag `rentProration:{moveInDay,daysOccupied,graced,capped,fullRent,proratedRent}`; recomputes totals by the rent delta. §7-E. Backward-compatible (no tenantData / later month = full). `writeBillOnIssue` passes tenantData; backfill doesn't.
- [x] ✅ **Read-only preview (§7-I/J gate)** — `tools/preview-movein-proration.js` (`npm run preview:movein-proration`) reuses the EXACT helpers over the Firebase CLI token (no drift). Run before deploy.
- [ ] **Admin override** — dashboard move-in-month bill row edits `charges.rent` (default = prorated) → write `rentAdjustment:{reason, by, defaultWas}` + `actionAudit`. *Why: standardise the case-by-case flexibility transparently (owner: "บางเคสขอลด/หักค่าปรับ/หักเต็ม").* **Best added after owner deploys + live-verifies the default.**
- [x] ✅ **Tests (+17 → suite 2480/2480)** — day 1 30-day → full; day 1 31-day → CAPPED full; day 15 → rent/30×17; `daysOccupied ≤ 5` → 0; utilities intact; non-move-in month + no-tenantData unaffected; future contractStart ignored (§7-BBB); leap Feb.
...
- **`00d5077` Phase 3 proration** — move-in-month rent daily proration in `functions/_billWrite.js` (rent/30 × daysOccupied, ≤5d grace, capped at 1 month; rent-only, utilities actual; `rentProration` metadata; backward-compatible). +17 tests → suite 2480/2480. Read-only §7-I/J gate `tools/preview-movein-proration.js`.
- **Confirmed (no change):** `deposits` rule `allow write: if isAdmin()` already covers reserved/lump client writes.

### Deferred / follow-ups
- **Phase 3 admin override** — the proration DEFAULT is shipped (`00d5077`); remaining = dashboard per-bill rent edit → `rentAdjustment` + actionAudit (UI, lower-risk; add after owner deploys + live-verifies).
...
- **At merge:** fold the proration into `billing_monthly_flow.md` (it's a billing rule) + promote the deposit-doc branch-only greps into the executable bash block.
