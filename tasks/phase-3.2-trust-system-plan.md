...
### 3.2c — Resident Rank · ✅ SHIPPED 2026-06-12 (PR1 #338 + PR2 #339)
Composite `round(0.40·reputation + 0.30·kindness + 0.30·verifiedHelper)` (**owner "สมดุล"**; tenure rides inside reputation, NOT a separate term) → 5-rung growth ladder `taproot|canopy|rooted|sprout|seed` (**owner ladder**: เมล็ดใหม่ → ต้นกล้า → ไม้ประจำถิ่น → ร่มเงาของตึก → รากแก้วชุมชน; bounds 75/55/35/15). Pure `functions/_residentRank.js` over the 3.2a+b outputs; recomputed in the same daily CF + mirrored as the 4th tier. Top rungs unreachable on reputation alone (cap 40) → participation lock-in. Drives the Emotional-Lock-in display. **Trust pillar #0/#6/#7/#8 COMPLETE.** Detail: [[lifecycle_trust_reputation]] (Resident Rank section).
